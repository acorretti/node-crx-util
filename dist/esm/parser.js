...
export function isCrx(crxPathOrBuffer) {
    const buffer = getBuffer(crxPathOrBuffer);
    const magic = buffer.readUInt32BE(0);
    return magic === 0x43723234; // Cr24
}
export function getCrxVersion(crxPathOrBuffer) {
    const buffer = getBuffer(crxPathOrBuffer);
    return buffer.readUInt32LE(4);
}
export function getZipContents(crxPathOrBuffer) {
    const buffer = getBuffer(crxPathOrBuffer);
    if (!isCrx(buffer)) {
...
export function extract(crxPathOrBuffer, output = process.cwd()) {
